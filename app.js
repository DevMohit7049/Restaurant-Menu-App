...
document.addEventListener("DOMContentLoaded", function () {
  ...
  menu.forEach((item) => {
    ...
    card.innerHTML = `<div class="menu-item col-12 col-md-6 col-lg-4">
      <div class="card">
...
const MainsBtn = document.getElementById("mains");
const StarterBtn = document.getElementById("starters");
const DessertBtn = document.getElementById("desserts");
const BrevrageBtn = document.getElementById("brevrages");
...
function UpdatedData(filteredData) {
...
AllBtn.addEventListener("click", function () {
  const allDish = menu.filter((item) => item.category);
  UpdatedData(allDish);
});

MainsBtn.addEventListener("click", function () {
  const MainDish = menu.filter((item) => item.category == "mains");
  UpdatedData(MainDish);
});

StarterBtn.addEventListener("click",function(){
  const StarterDish = menu.filter((item)=>item.category=="starters");
  UpdatedData(StarterDish);
});

DessertBtn.addEventListener("click",function(){
   const DessertDish = menu.filter((item)=>item.category=='desserts');
   UpdatedData(DessertDish);
});

BrevrageBtn.addEventListener("click",function(){
  const Brevrages = menu.filter((item)=>item.category=='beverages');
  UpdatedData(Brevrages);
});

SearchInput.addEventListener('keypress',()=>{
   const searchingData = SearchInput.value.toLowerCase();
   const SearchDish = menu.filter((item)=>item.category.toLowerCase().includes(searchingData));
   UpdatedData(SearchDish);
});
